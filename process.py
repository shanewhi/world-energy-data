...
########################################################################################
#
# Function: id_large_ffco2_emitters()
#
# Description:
# Identify nations emitting more FF CO2 than a preset threshold.
#
########################################################################################
def id_large_ffco2_emitters(global_carbon: object) -> tuple[Any, Any]:
	large_emitters = global_carbon.national_shares_fy[
		global_carbon.national_shares_fy[
			"Value"] >= user_globals.Constant.LARGE_EMITTING_NATION_THRESHOLD.value
		]
	large_emitters.set_index("Name", inplace=True)
	large_emitters = large_emitters.drop("Other")

	# large_emitters_share = round(sum(largest_emitting_nations["Value"]), 1)

	return large_emitters.index.to_list()
