...
def country_co2_charts(energy_system, global_carbon):
    ...
    subplot2_title = "Year " + str(int(max(global_carbon.national_shares_fy_dataframe["Year"])))

    large_emitter_share_total = 100 - float(
        global_carbon.national_shares_fy_dataframe[
            global_carbon.national_shares_fy_dataframe["Name"] == "Other"
            ].Value.values
    )
    one_percent_countries = global_carbon.national_shares_fy_dataframe[
        global_carbon.national_shares_fy_dataframe["Value"] >= 1
        ]
    ...
    half_to_one_percent_countries = global_carbon.national_shares_fy_dataframe.loc[
        global_carbon.national_shares_fy_dataframe["Value"] < 1
        ].Name.values
...
Segments excluding 'Other' represent\ncountries with a "
            ...
            + "% or greater share,\nof which there were "
            + str(len(global_carbon.national_shares_fy_dataframe) - 1)
            + ", totalling "
            + str(large_emitter_share_total)
            + "%.\n\n\nLabelled segments are those with a 1%\nor greater share, of which there were\n"
            + str(
        len(global_carbon.national_shares_fy_dataframe[global_carbon.national_shares_fy_dataframe["Value"] >= 1]) - 1)
...
https://www.energyinst.org/statistical-review/resources-and-data-downloads.")
    ...
    print(
        ...
        + str(sum(global_carbon.national_shares_fy_dataframe["Value"]))
        + "%"
    )
    if user_globals.Constant.DISPLAY_CHARTS.value is True:
        plt.show()
