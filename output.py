...
#
# Description:
# Controls plotting of country CO2 emissions from fossil fuel combustion and national share treemap
#
########################################################################################################################
...
#
# Description:
# Controls plotting of CO2 emissions by sector using IEA data.
#
########################################################################################################################
def co2_by_sector_chart(energy_system):
    fig_dir = 'charts ' + energy_system.country + '/'
    os.makedirs(fig_dir, exist_ok=True)  # Save co2 charts in this directory.
    ...
    title5 = 'Commercial & Public Services ' + str(
        energy_system.sector_co2_Mt['Commercial & Public Services Share'].iloc[-1]) + '%'
    title6 = 'Residences ' + str(energy_system.sector_co2_Mt['Residential Share'].iloc[-1]) + '%'
    title7 = 'Ag, Forestry & Fishing ' + str(energy_system.sector_co2_Mt['Ag, Forestry & Fishing Share'].iloc[-1]) + '%'
    title8 = 'Other ' + str(energy_system.sector_co2_Mt['Non-specified (Other) Share'].iloc[-1]) + '%'
    ...
    sec6 = energy_system.sector_co2_Mt['Residences']
    sec7 = energy_system.sector_co2_Mt['Ag, Forestry & Fishing']
    sec8 = energy_system.sector_co2_Mt['Non-specified (Other)']
    ...
    footer_text = (str(energy_system.sector_co2_Mt.index[-1]) + ' values: '
                   ...
                   + 'Mt, Transport = '
                   + f'{(round(energy_system.sector_co2_Mt['Transport'].iloc[-1])):,}'
                   + 'Mt, Commercial & Public Services = '
                   + f'{(round(energy_system.sector_co2_Mt['Commercial & Public Services'].iloc[-1])):,}'
                   + 'Mt, Residences = '
                   + f'{(round(energy_system.sector_co2_Mt['Residences'].iloc[-1])):,}'
                   + 'Mt, Ag, Forestry & Fishing = '
                   + f'{(round(energy_system.sector_co2_Mt['Ag, Forestry & Fishing'].iloc[-1])):,}'
                   + 'Mt, Other = '
...
WORLD_GHG_Documentation_2024_final.pdf')
    ...
    chart.column_8_subplots(
        ...
        sec7,
        sec8,
        user_globals.Color.SECTOR_ELEC.value,
        user_globals.Color.SECTOR_OTHER_ENERGY.value,
        user_globals.Color.SECTOR_INDUSTRY.value,
        user_globals.Color.SECTOR_TRANSPORT.value,
        user_globals.Color.SECTOR_COM_PUBLIC_SERV.value,
        user_globals.Color.SECTOR_RESD.value,
        user_globals.Color.SECTOR_AG.value,
        user_globals.Color.SECTOR_OTHER.value,
        country,
        title,
        title1,
        title2,
...
def world_ffprod_stacked(country_folder_name, major_ffprod_data):
    ...
    title = ('Fossil Fuel Production Trends of Countries with ≥2% National Share in ' + str(final_yr) +
             ' (share value shown after country name)')
...
def country_finalenergy_elec_charts(energy_system):
    country = energy_system.country
    if energy_system.finalenergy_PJ is None:
        return None
    fig_dir = 'charts ' + country + '/'
    os.makedirs(fig_dir, exist_ok=True)  # Save co2 charts in this directory.
    ####################################################################################################################
...
def country_finalenergy_charts(energy_system):
    ...
    if country == 'World':
        ...
        footer_text = (str(energy_system.finalenergy_PJ.index[-1]) + ' values: '
                       + 'Coal = '
                       + f'{(round(energy_system.finalenergy_PJ['Coal'].iloc[-1])):,}'
                       + 'EJ, Oil = '
                       + f'{(round(energy_system.finalenergy_PJ['Oil'].iloc[-1])):,}'
                       + 'EJ, Gas = '
                       + f'{(round(energy_system.finalenergy_PJ['Gas'].iloc[-1])):,}'
                       + 'EJ, Biofuels & Waste = '
                       + f'{(round(energy_system.finalenergy_PJ['Biofuels and Waste'].iloc[-1])):,}'
                       + 'EJ, Electricity = '
                       + f'{(round(energy_system.finalenergy_PJ['Electricity'].iloc[-1])):,}'
                       + 'EJ, Heat = '
                       + f'{(round(energy_system.finalenergy_PJ['Heat'].iloc[-1])):,}'
                       + 'EJ\nQuantities of coal, oil, gas, biofuels, and waste shown were consumed for purposes \
other than electricity generation, such as steel manufacture, internal combustion, cooking, etc.\n\
By Shane White, whitesha@protonmail.com, https://github.com/shanewhi/world-energy-data. \n\
Data: IEA World Energy Balances, https://www.iea.org/data-and-statistics/data-tools/energy-statistics-data-browser?\
country=WORLD&fuel=Energy%20consumption&indicator=TFCbySource.')
        ...
        footer_text = (str(energy_system.finalenergy_PJ.index[-1]) + ' values: '
                       ...
                       + 'PJ, Heat = '
                       + f'{(round(energy_system.finalenergy_PJ['Heat'].iloc[-1])):,}'
                       + 'PJ\nQuantities of coal, oil, gas, biofuels, and waste shown were consumed for purposes other \
than electricity generation, such as steel manufacture, internal combustion, cooking, etc.\n\
By Shane White, whitesha@protonmail.com, https://github.com/shanewhi/world-energy-data. \n\
Data: IEA World Energy Balances, https://www.iea.org/data-and-statistics/data-tools/energy-statistics-data-browser?\
country=WORLD&fuel=Energy%20consumption&indicator=TFCbySource.')
...
Quantities are gross generation that don't account for imports or exports.\n"
                       ...
                       + ' values (rounded): \n'
                       + 'Total = '
                       + f'{(round(energy_system.elecgen_TWh['Total Country'].iloc[-1], 1)):,}'
                       + ' TWh\nNuclear = '
                       + f'{(round(energy_system.elecgen_TWh['Nuclear'].iloc[-1], 1)):,}'
                       + ' TWh\nFossil Fuels = '
                       + f'{(round(energy_system.elecgen_TWh['Fossil Fuels'].iloc[-1], 1)):,}'
                       + ' TWh\nCoal = '
                       + f'{(round(energy_system.elecgen_TWh['Coal'].iloc[-1], 1)):,}'
                       + ' TWh\nOil = '
                       + f'{(round(energy_system.elecgen_TWh['Oil'].iloc[-1], 1)):,}'
                       + ' TWh\nGas = '
                       + f'{(round(energy_system.elecgen_TWh['Gas'].iloc[-1], 1)):,}'
                       + ' TWh\nBio, Geo and Other = '
                       + f'{(round(energy_system.elecgen_TWh['Bio, Geo and Other'].iloc[-1], 1)):,}'
                       + ' TWh\nRenewables = '
                       + f'{(round(energy_system.elecgen_TWh['Renewables'].iloc[-1], 1)):,}'
                       + ' TWh\nHydro = '
                       + f'{(round(energy_system.elecgen_TWh['Hydro'].iloc[-1], 1)):,}'
                       + ' TWh\nWind = '
...
https://www.energyinst.org/statistical-review/resources-and-data-downloads.')
        ...
        chart.column_grouped_2_subplots(
            ...
            footer_text,
            user_globals.Color.ELECTRICITY.value,
            energy_system.elecgen_TWh['Total Change'],
            user_globals.Color.COAL.value,
            user_globals.Color.OIL.value,
            user_globals.Color.GAS.value,
            user_globals.Color.NUCLEAR.value,
